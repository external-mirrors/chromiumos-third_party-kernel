				    ...
				    struct avs_kcontrol_tlv_data *kctrl_tlv_data)
{
	...
	for (i = 0; i < kctrl_tlv_data->count; i++) {
		...
		tlv = (struct avs_tlv *)((u8*)tlv + 2 * sizeof(u32) + tlv->length);
	}
			       ...
			       unsigned int size)
{
	...
	check_size = 2 * sizeof(u32); /* TL of kctrl_tlv_data */
	for (i = 0; i < kctrl_tlv_data->count; i++) {
		check_size += 2 * sizeof(u32) + atlv->length; /* V[i] of kctrl_tlv_data */
		if (check_size > tlv.length) {
			ret = -EINVAL;
			goto err;
		}
		atlv = (struct avs_tlv *)((u8*)atlv + 2 * sizeof(u32) + atlv->length);
	}
