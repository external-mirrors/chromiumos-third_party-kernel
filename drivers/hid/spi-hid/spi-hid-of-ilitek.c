...
int ili_ic_check_int_level(bool level)
{
	...
	ILI_INFO("++++ ili_ic_check_int_level ++++\n");
	/*
	 * If callers have a trouble to use the gpio that is passed by vendors,
	 * please utilises a physical gpio number instead or call a help from them.
	 */
...
int ili_cascade_ic_get_info(bool enter_ice, bool exit_ice, bool mcu, bool reset)
{
	...
	if (((ilits->chip->client_pid >> 28) & 0xF) == 0xF) {
		...
		ili_spi_ice_mode_read(TDDI_SECOND_PID_ADDR, &ilits->chip->client_second_pid, sizeof(u32), CLIENT);
	}
	...
	if (((ilits->chip->pid & 0xFFFFFF00) != (ilits->chip->client_pid & 0xFFFFFF00)) ||
			(needSecondClientChipID && ((ilits->chip->second_pid & 0xFFFF) != (ilits->chip->client_second_pid & 0xFFFF)))) {
		...
		if ((fbi[i].mode & (0x01 << mode)) && fbi[i].len != 0) {
			...
			if (ilits->cascade_info_block.nNum != 0) {
				...
				ILI_INFO("Client %s CRC is %s hex(%x) : dma(%x) : iram(%x)\n",
					fbi[i].name, (dma_crc_err_client) ? "Invalid !" : "Correct !", hex_crc, dma, iram_crc);
			}
...
static int ilitek_tddi_fw_update_block_info(u8 *pfw)
{
	...
	if (tfd.mapping_tag == BLOCK_TAG_B0) {
		...
		fbi[DATA_BLOCK].mem_start = (fbi[DATA_BLOCK].fix_mem_start != INT_MAX) ? fbi[DATA_BLOCK].fix_mem_start : DLM_START_ADDRESS;
		fbi[TUNING].mem_start = (fbi[TUNING].fix_mem_start != INT_MAX) ? fbi[TUNING].fix_mem_start :  fbi[DATA_BLOCK].mem_start + fbi[DATA_BLOCK].len;
		...
		/* upgrade mode define */
		fbi[DATA_BLOCK].mode = fbi[AP].mode = fbi[TUNING].mode = fbi[PEN].mode = 0x01 << NEED_UPGRADE_AP;
			...
			for (j = 0; j < DEFINED_MODE_NUM; j++) {
				if (fbi[i].mode & (0x01 << j)) {
					...
					case DATA_BLOCK:
						...
						break;
					case TUNING:
						blk_iram_addr_default = fbi[DATA_BLOCK].fix_mem_start_multi_buf[j] + fbi[DATA_BLOCK].len;
						break;
					case GESTURE:
	...
	ILI_INFO("report_info_block : nReportByPixel = %d, nIsHostDownload = %d, nIsSPIICE = %d, nIsSPICLIENT = %d, nIsI2C = %d\n",
		ilits->rib.nReportByPixel, ilits->rib.nIsHostDownload, ilits->rib.nIsSPIICE, ilits->rib.nIsSPICLIENT, ilits->rib.nIsI2C);
...
#endif
#else
static int ili_spi_write_then_read_direct(struct spi_device *spi,
		const void *txbuf, unsigned n_tx,
		void *rxbuf, unsigned n_rx)
{
	int status = -1, duplex_len = 0;
...
int ilitek_spi_probe(struct spi_device *client)
{
	...
	ilits = devm_kzalloc(&spi->dev, sizeof(struct ilitek_ts_hid_data), GFP_KERNEL);
	if (ERR_ALLOC_MEM(ilits)) {
	...
	ilits->input_reg_state = INPUT_DEV_STATUS_UNREGISTER;
	ilits->tp_notifier_reg_status = TP_NOTIFIER_STATUS_UNREGISTER;
	atomic_set(&ilits->init_stat, START);
...
int ili_cascade_rw_tp_reg(u32 mcu, u32 type, u32 addr, u32 wdata, u32 wlen, u32 *rdata, bool bypass_exit_ice)
{
	...
	if (type == read) {
		ili_spi_ice_mode_read(addr, &read_data, sizeof(u32), ilits->client_wr_ctrl);
		*rdata = read_data;
...
{
	int len = 0;
	ILI_DBG("type = %d, client_wr_ctrl = %d\n", type, ilits->client_wr_ctrl);
	if (type == ILI_WRITE && ilits->client_wr_ctrl == CLIENT) {
		ili_ice_client_write_register(addr, wdata, wlen);
		ILI_INFO("[WRITE]:addr = 0x%06x, write = 0x%08x, len = %d byte\n", addr, wdata, wlen);
	} else {
				     ...
				     unsigned long arg)
{
	...
	case ILITEK_COMPAT_IOCTL_CLIENT_CTRL:
		ILI_DBG("compat_ioctl: lcm status\n");
		ret = filp->f_op->unlocked_ioctl(filp, ILITEK_IOCTL_CLIENT_CTRL, (unsigned long)compat_ptr(arg));
		return ret;
	...
	case ILITEK_COMPAT_IOCTL_ICE_MODE_FLAG_SET:
		...
		return ret;
	case ILITEK_COMPAT_IOCTL_SWITCH_ICE_FLASH_CS_CTL:
		ILI_DBG("compat_ioctl: switch ice flash cs control\n");
		ret = filp->f_op->unlocked_ioctl(filp, ILITEK_IOCTL_SWITCH_ICE_FLASH_CS_CTL, (unsigned long)compat_ptr(arg));
		return ret;
	case ILITEK_COMPAT_IOCTL_MP_RUNNING_BY_DAEMON:
		ILI_DBG("compat_ioctl: mp running by daemon\n");
		ret = filp->f_op->unlocked_ioctl(filp, ILITEK_IOCTL_MP_RUNNING_BY_DAEMON, (unsigned long)compat_ptr(arg));
		return ret;
	default:
			      ...
			      unsigned long arg)
{
	...
	bool read = 0;
	u32 type = 0, addr = 0, read_data = 0, write_data = 0, write_len = 0;
	...
	case ILITEK_IOCTL_ICE_MODE_SWITCH:
		...
		if (ilits->cascade_info_block.nNum != 0) {
			...
			if (ilits->client_wr_ctrl == ENABLE && szBuf[0] == ENABLE) {
	...
	case ILITEK_IOCTL_TP_FW_UART_CTRL:
		ILI_INFO("This node is not supported\n");
		break;
	...
	case ILITEK_IOCTL_CLIENT_CTRL:
		...
		ILI_DBG(" %s client write read ctrl = %x\n", ilits->client_wr_ctrl ? "Enable" : "Disable", ilits->client_wr_ctrl);

#if (ENABLE_CASCADE)
		ilits->i2c_addr_change = (ilits->client_wr_ctrl == CLIENT) ? CLIENT : MASTER;
#endif
		break;
	case ILITEK_IOCTL_CASCADE_REG_RW:
		...
		ILI_DBG("type = %d, addr = 0x%X, write_data = %X, write_len = %d\n",
				type, addr, write_data, write_len);

		if (type == read) {
			ili_ice_mode_read_by_mode(addr, &read_data, sizeof(u32), ilits->client_wr_ctrl);
			...
			ili_ice_mode_write_by_mode(addr, write_data, write_len, ilits->client_wr_ctrl);
		}
	break;
	...
	case ILITEK_IOCTL_SWITCH_ICE_FLASH_CS_CTL:
		...
		break;
	case ILITEK_IOCTL_MP_RUNNING_BY_DAEMON:
		if (copy_from_user(szBuf, (u8 *) arg, 1)) {
			ILI_ERR("Failed to copy data from user space\n");
			ret = -ENOTTY;
			break;
		}
		ILI_DBG("ioctl: mp running by daemon = %d\n", szBuf[0]);
...
int ilitek_plat_gpio_register(void)
{
	...
	ilits->spi->irq = of_irq_get(dev->of_node, 0);
	if (ilits->spi->irq < 0) {
		ILI_ERR("Failed to get irq from DT: %d\n", ilits->spi->irq);
		return ilits->spi->irq;
	}
...
void ili_irq_disable(void)
{
	...
	if (atomic_read(&ilits->irq_stat)) {
		disable_irq_nosync(ilits->spi->irq);
		atomic_set(&ilits->irq_stat, DISABLE);
	}
	ILI_INFO("Disable irq success\n");
...
void ili_irq_enable(void)
{
	...
	if (!(atomic_read(&ilits->irq_stat))) {
		enable_irq(ilits->spi->irq);
		atomic_set(&ilits->irq_stat, ENABLE);
	}
	ILI_INFO("Enable irq success\n");
...
int ilitek_notifier_register(void)
{
	...
	ilits->tp_notifier_reg_status = TP_NOTIFIER_STATUS_REGISTER;
	ILI_INFO("TP_notifier register on disp_notifier chain\n");
#endif
...
{
	ILI_INFO("Unregister TP_notifier on disp_notifier chain\n");

#ifdef CONFIG_AMLOGIC_LEGACY_EARLY_SUSPEND
	unregister_early_suspend(&suspend_handler);
	ilits->tp_notifier_reg_status = TP_NOTIFIER_STATUS_UNREGISTER;
#endif
}
...
int ilitek_plat_remove(void)
{
	...
	if (ilits->tp_notifier_reg_status == TP_NOTIFIER_STATUS_REGISTER) {
		ilitek_notifier_unregister();
	}
...
int ili_sleep_handler(int mode)
{
	int ret = 0;
	int wake_status = 0;
	struct spi_device *spi_dev = ilits->spi;
	struct spi_hid *shid = spi_get_drvdata(ilits->spi);

	if (atomic_read(&ilits->init_stat)) {
		ILI_INFO("HID enumeration not completed yet, init_stat = %d\n", atomic_read(&ilits->init_stat));
		return 0;
	}
	...
	if (atomic_read(&ilits->fw_stat) || atomic_read(&ilits->mp_stat)) {
		ILI_INFO("ignore request, fw_stat = %d, mp_stat = %d\n",
				atomic_read(&ilits->fw_stat), atomic_read(&ilits->mp_stat));
	...
	case TP_RESUME:
		...
		if (ilits->irq_wake_enabled) {
			wake_status = disable_irq_wake(spi_dev->irq);
			if (!wake_status)
...
int ili_set_tp_data_len(int format, bool send, u8 *data)
{
	...
	case DATA_FORMAT_GESTURE_SPECIAL_DEMO:
		...
		ili_ic_func_ctrl("gesture_demo_en",
				     ilits->gesture_demo_ctrl);
...
int ili_set_pen_data_len(u8 header, u8 ctrl, u8 type)
{
	...
	ILI_INFO("Pen Type = 0x%X, Pen Data Mode = %d\n", ilits->pen_info.report_type,
		ilits->pen_info.pen_data_mode);
...
int ili_tddi_init(void)
{
	...
	atomic_set(&ilits->ignore_report, END);
	atomic_set(&ilits->irq_stat, ENABLE);
	atomic_set(&ilits->irq_after_recovery, END);
	...
	do {
		...
		if (ilits->ice_mode_ctrl(DISABLE, OFF, BOTH) < 0)
			ILI_ERR("Failed to disable ice mode during ili_tddi_init\n");
...
{
	struct spi_hid_of_ilitek_config *config = container_of(ops, struct spi_hid_of_ilitek_config, ops);
	...
	if (ret) {
		dev_err(dev, "Failed to transfer device_reset_request.");
	}
...
static int spi_hid_of_delayed_probe(struct spi_device *spi)
{
	...
	config = devm_kzalloc(dev, sizeof(struct spi_hid_of_ilitek_config),
			      GFP_KERNEL);
	if (!config)
		return -ENOMEM;
	...
	atomic_set(&ilits->init_stat, END);
...
static int spi_hid_of_ilitek_probe(struct spi_device *spi)
{
	...
	dp = devm_kzalloc(&spi->dev, sizeof(*dp), GFP_KERNEL);
	if (!dp)
		return -ENOMEM;

	dp->dev = &spi->dev;
	dp->spi = spi;
	dp->core_probed = false;

	spi_set_drvdata(spi, dp);

	INIT_DELAYED_WORK(&dp->dwork, ilitek_delayed_work);
	schedule_delayed_work(&dp->dwork, msecs_to_jiffies(4000));
...
{
	struct ilitek_delayed_probe *dp = spi_get_drvdata(spi);

	if (!dp)
		return;

	cancel_delayed_work_sync(&dp->dwork);
	dp->spi = NULL;
