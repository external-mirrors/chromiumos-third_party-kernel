					...
					struct acpi_device *adev)
{
	...
	/* Revision 3 for HID over SPI V1, see dpecification. */
...
static int spi_hid_acpi_power_up(struct spihid_ops *ops)
{
	...
	ret = acpi_device_set_power(conf->adev, ACPI_STATE_D0);
	if (ret) {
		dev_err(&conf->adev->dev, "Error could not power up ACPI device: %d.", ret);
		return ret;
	}
...
static int spi_hid_acpi_probe(struct spi_device *spi)
{
	...
	ret = spi_hid_acpi_populate_config(config, adev);
	if (ret) {
