...
#define SPI_HID_RESPONSE_TIMEOUT	1000
...
static int spi_hid_suspend(struct spi_hid *shid)
{
	...
	if (!device_may_wakeup(dev)) {
		...
		ret = shid->ops->power_down(shid->ops);
		if (ret) {
...
static int spi_hid_resume(struct spi_hid *shid)
{
	...
	if (!device_may_wakeup(dev)) {
		...
		ret = shid->ops->power_up(shid->ops);
		if (ret) {
	...
	ret = hid_driver_reset_resume(shid->hid);
	if (ret)
				...
				struct spi_hid_output_report *report)
{
	...
	ret = wait_for_completion_interruptible_timeout(&shid->output_done,
							msecs_to_jiffies(shid->response_timeout));
...
static ssize_t response_timeout_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct spi_hid *shid = dev_get_drvdata(dev);
	int new_val, err;

	err = kstrtoint(buf, 10, &new_val);
	if (err < 0)
		return err;

	if (new_val < 1000 || new_val > 10000)
		return -EINVAL;

	dev_dbg(&shid->spi->dev, "Updating response timeout from %d to %d",
		shid->response_timeout, new_val);
...
static DEVICE_ATTR_WO(response_timeout);
...
static struct attribute *spi_hid_attrs[] = {
	...
	&dev_attr_response_timeout.attr,
	NULL	/* Terminator */
};
		       ...
		       struct spi_hid_conf *conf)
{
	...
	shid->ops = ops;
	shid->conf = conf;
	shid->response_timeout = SPI_HID_RESPONSE_TIMEOUT;
