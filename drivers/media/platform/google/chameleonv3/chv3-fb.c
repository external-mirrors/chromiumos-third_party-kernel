...
static void chv3_fb_set_default_format(struct chv3_fb *fb, int legacy_format)
{
...
static int chv3_fb_probe(struct platform_device *pdev)
{
	...
	if (device_property_read_bool(&pdev->dev, "google,legacy-format"))
		legacy_format = 1;

	if (device_property_read_bool(&pdev->dev, "google,no-endpoint"))
		no_endpoint = 1;
