...
static const enum base_hw_issue *kbase_hw_get_issues_for_new_id(struct kbase_device *kbdev)
{
	...
	/* Replacing gpu_id with original gpu_id to identify the special G-57 on
	 * MT81XX, and reconfigure its base hw issues.
	 */
	struct kbase_gpu_id_props *gpu_id = &kbdev->gpu_props.orig_gpu_id;
