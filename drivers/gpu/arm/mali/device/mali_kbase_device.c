...
int kbase_device_early_init(struct kbase_device *kbdev)
{
	...
	if (!kbase_reg_is_init(kbdev)) {
		 ...
		 */
		orig_gpu_id = kbase_reg_get_gpu_id(kbdev);
		gpu_id = orig_gpu_id;
		if ((orig_gpu_id & (GPU_ID2_PRODUCT_MODEL | GPU_ID2_VERSION_MAJOR |
				    GPU_ID2_ARCH_REV)) == GPU_ID2_PRODUCT_MAKE(9, 0, 9, 3)) {
			gpu_id = GPU_ID2_MAKE(9, 0, 9, 1, GPU_ID2_VERSION_MAJOR_GET(orig_gpu_id),
					      GPU_ID2_VERSION_MINOR_GET(orig_gpu_id), 0);
			dev_info(kbdev->dev,
		...
		kbase_gpuprops_parse_gpu_id(&kbdev->gpu_props.gpu_id, gpu_id);
		kbase_gpuprops_parse_gpu_id(&kbdev->gpu_props.orig_gpu_id, orig_gpu_id);
