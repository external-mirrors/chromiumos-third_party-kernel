...
static struct drm_pending_event *create_update_ready_event(void)
{
	...
	event = kzalloc_obj_evdi(*event, GFP_KERNEL);
	if (!event) {
...
static void evdi_add_i2c_adapter(struct evdi_device *evdi)
{
	...
	evdi->i2c_adapter = kzalloc_obj_evdi(*evdi->i2c_adapter, GFP_KERNEL);
...
{
	EVDI_CHECKPT();
	dev->painter = kzalloc_obj_evdi(*dev->painter, GFP_KERNEL);
	if (dev->painter) {
