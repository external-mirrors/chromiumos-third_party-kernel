 ...
 */
static bool
drm_edp_set_dynamic_backlight_percent(struct drm_dp_aux *aux, u32 min, u32 max)
{
	u8 dbc[] = { DIV_ROUND_CLOSEST(min, 5), DIV_ROUND_CLOSEST(max, 5) };

	if (drm_dp_dpcd_write(aux, DP_EDP_DBC_MINIMUM_BRIGHTNESS_SET, dbc,
			      sizeof(dbc)) < 0) {
		drm_dbg_kms(aux->drm_dev,
			     ...
			     const u16 level)
{
	...
	if (bl->use_dbc && bl->dbc_supported) {
		if (drm_edp_set_dynamic_backlight_percent(aux, 0, 100)) {
		       ...
		       u16 *current_level, u8 *current_mode)
{
	...
	if (edp_dpcd[2] & DP_EDP_DYNAMIC_BACKLIGHT_CAP)
		bl->dbc_supported = true;
