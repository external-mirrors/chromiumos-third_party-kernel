...
#define TX_REG_AUD_FULLPKT 0xB9
...
struct it61620 {
	...
	struct i2c_client *cec_i2c;
	struct regmap *it61620_regmap;
	struct regmap *mipirx_regmap;
	struct regmap *tx_regmap;
	struct regmap *cec_regmap;

	struct delayed_work hdcp_work;
	struct delayed_work plugged_cb_work;
...
static void it61620_stop_plugged_cb_work(struct it61620 *it61620);
static inline void it61620_hdmi_enable_avmute(struct it61620 *it61620, bool enable);
static void it61620_hdmi_hdcp_wait_ksv_list(struct it61620 *it61620);
static void it61620_audio_update_connector_status(struct it61620 *it61620);
...
static const struct regmap_config it61620_cec_regmap_config = {
	.reg_bits = 8,
	.val_bits = 8,
	.max_register = 0xff,
	.cache_type = REGCACHE_NONE,
...
static unsigned int it61620_mipi_reg_read(struct it61620 *it61620, unsigned int reg)
{
	unsigned int val;
	int err;
...
static unsigned int it61620_hdmi_reg_read(struct it61620 *it61620, unsigned int reg)
{
	unsigned int val;
	int err;
	struct device *dev = it61620->dev;
...
static void it61620_mipi_m2p_reset(struct it61620 *it61620)
{
	if (it61620->dev_ver != DEV_VERSION_A0)
		return;
	it61620_mipi_reg_set(it61620, RX_REG_MIPI_CTRL02, B_FIFO_RST, B_FIFO_RST);
	usleep_range(1000, 2000);
...
static void it61620_mipi_reset(struct it61620 *it61620)
{
	...
	it61620_mipi_reg_set(it61620, RX_REG_RESET_CTRL, B_REF_CLOCK_RESET,
			     B_REF_CLOCK_RESET);
	usleep_range(1000, 2000);
	it61620_mipi_reg_set(it61620, RX_REG_RESET_CTRL, B_REF_CLOCK_RESET, 0x00);
	usleep_range(1000, 2000);
	...
	if (dev_ver != DEV_VERSION_A0) {
		...
		it61620_mipi_reg_set(it61620, RX_REG_FORCE_M, B_FORCE_MVT_STB, B_FORCE_MVT_STB);
...
static void it61620_mipi_irq_handler(struct it61620 *it61620)
{
	...
	reg_val = it61620_mipi_reg_read(it61620, RX_REG_CLK_CTRL) & B_MIPI_INT_STATUS;
	if (reg_val == 0)
		return;
...
static void it61620_mipi_set_d2v_video_timing(struct it61620 *it61620)
{
	...
	u32 clock;
	bool hpol_high = false, vpol_high = false;
	...
	it61620_mipi_reg_set(it61620, RX_REG_POL, 0x03, ((!vpol_high) << 1) | (!hpol_high));

	if (it61620->dev_ver == DEV_VERSION_A0 &&
	    ((hdew == 2560 && vdew == 1440 && clock == 241500) ||
	     (hdew == 720 && vdew == 400 && clock == 28320) ||
	     (hdew == 720 && vdew == 576 && clock == 27000))) {
		dev_dbg(it61620->dev, "device A0 %dx%d %d Khz", hdew, vdew, clock);
...
static bool it61620_hdmi_enable_hdcp(struct it61620 *it61620)
{
	struct drm_device *drm = it61620->drm;
	...
	it61620_hdmi_reg_set(it61620, TX_REG_HDCP_CTRL2, B_EN_AN_GEN, B_EN_AN_GEN);
	usleep_range(1000, 2000);
	it61620_hdmi_reg_set(it61620, TX_REG_HDCP_CTRL2, B_EN_AN_GEN, 0x00);

	it61620_hdmi_reg_set(it61620, TX_REG_HDCP_CTRL1, B_CPDESIRED, B_CPDESIRED);
	it61620_hdmi_reg_set(it61620, TX_REG_HDCP_CTRL3, (B_PAUSE | B_ENC_DIS), 0x00);

	it61620_hdmi_reg_write(it61620, TX_REG_HDCP_PRO, B_AUTH_FIRE);

	/*
	 * HDCP start requires 100ms for the first part of the authentication protocol,
	 * 20ms is used to wait for 61620 HW completion.
	 */
	...
	if (auth_cs == CS_AUTH_DONE) {
		...
		it61620_hdmi_reg_set(it61620, TX_REG_INT1, B_INT_AUTH_F, B_INT_AUTH_F);
		it61620_hdmi_reg_set(it61620, TX_REG_INT_MASK1, B_INT_AUTH_F, B_INT_AUTH_F);
...
static bool it61620_hdmi_start_hdcp(struct it61620 *it61620)
{
	struct drm_device *drm = it61620->drm;
	u8 rx_hdmi_mode;
	...
	if (it61620_hdmi_hdcprd(it61620, DRM_HDCP_DDC_BSTATUS, DRM_HDCP_BSTATUS_LEN) < 0) {
...
static int it61620_hdmi_setup_sha1_input(struct it61620 *it61620, u8 *input)
{
	...
	for (i = 0; i < 8; i++)
		input[count++] = it61620_hdmi_reg_read(it61620, (TX_REG_ANM0V1 + i));
...
static bool it61620_hdmi_hdcp_part2_ksvlist_check(struct it61620 *it61620)
{
	...
	for (i = 0; i < DRM_HDCP_V_PRIME_NUM_PARTS; i++) {
		it61620_hdmi_hdcprd(it61620, DRM_HDCP_DDC_V_PRIME(i), DRM_HDCP_V_PRIME_PART_LEN);
		...
		for (j = 0; j < DRM_HDCP_V_PRIME_PART_LEN; j++)
			bv[i][j] = it61620_hdmi_reg_read(it61620, TX_REG_ANM0V1 + j);
	}
...
static int it61620_hdmi_avi_infoframe_set(struct it61620 *it61620)
{
	struct drm_device *drm = it61620->drm;
	u8 buf[32], i, *ptr;
	int ret;
	...
	ret = hdmi_avi_infoframe_pack(&it61620->avi_info, buf, sizeof(buf));
	if (ret < 0) {
		drm_dbg(drm, "Failed to pack AVI infoframe: %d", ret);
		return ret;
	}

	/* fill PB */
	ptr = buf + HDMI_INFOFRAME_HEADER_SIZE;
	for (i = 0; i < it61620->avi_info.length; i++)
		it61620_hdmi_reg_write(it61620, TX_REG_AVIINFO_DB01 + i, ptr[i]);

	it61620_hdmi_reg_write(it61620, TX_REG_AVIINFO_DB14, buf[3]);
...
static int it61620_hdmi_audio_infoframe_set(struct it61620 *it61620, int length)
{
	u8 *buf, i, *ptr;
	...
	buf = it61620->audio_config.infoframe;
	/* fill PB */
	ptr = buf + HDMI_INFOFRAME_HEADER_SIZE;
	for (i = 0; i < length; i++)
		it61620_hdmi_reg_write(it61620, TX_REG_AUDINFO_DB01 + i, ptr[i]);

	it61620_hdmi_reg_write(it61620, TX_REG_AUDINFO_DB06, buf[3]);
...
static int it61620_enable_devices(struct it61620 *it61620)
{
	...
	for (i = 0; i < 5; i++)
		device_id[i] = it61620_mipi_reg_read(it61620, REG_VENDER_ID_L + i);
				  ...
				  size_t len)
{
	...
	for (i = 0; i < EDID_LENGTH; i += EDID_R_BURST_NUM,
		...
		it61620_hdmi_reg_write(it61620, TX_REG_DDC_NUM_L, EDID_R_BURST_NUM);
		it61620_hdmi_reg_write(it61620, TX_REG_DDC_NUM_H,
				       (EDID_R_BURST_NUM >> 8));
		it61620_hdmi_reg_write(it61620, TX_REG_DDC_COMMAND, DDC_COMMAND_EDID_RD);
...
static void it61620_config_audio(struct it61620 *it61620)
{
	...
	it61620_hdmi_reg_write(it61620, TX_REG_AUDINFO_DB01,
			       config->channel_count - 1);
	it61620_hdmi_reg_write(it61620, TX_REG_AUDINFO_DB02, 0x00);

	it61620_hdmi_reg_write(it61620, TX_REG_AUDINFO_DB03, 0x00);
	it61620_hdmi_reg_write(it61620, TX_REG_AUDINFO_DB04, infoca);
	it61620_hdmi_reg_write(it61620, TX_REG_AUDINFO_DB05, 0x00);
	...
	it61620_hdmi_reg_write(it61620, TX_REG_AUDINFO_DB06, 0x100 - chksum);
...
static void it61620_audio_update_connector_status(struct it61620 *it61620)
{
	...
	status = it61620->connector_status;
	if (it61620->plugged_cb && it61620->codec_dev)
		it61620->plugged_cb(it61620->codec_dev,
...
static int it61620_hdmi_hw_params(struct device *dev, void *data,
				  struct hdmi_codec_daifmt *fmt,
				  struct hdmi_codec_params *hparms)
{
...
static int it61620_hdmi_audio_startup(struct device *dev, void *data)
{
	struct it61620 *it61620 = dev_get_drvdata(dev);
...
static void it61620_hdmi_audio_shutdown(struct device *dev, void *data)
{
	struct it61620 *it61620 = dev_get_drvdata(dev);
...
static int it61620_hdmi_audio_hook_plugged_cb(struct device *dev,
					      void *data,
					      ...
					      struct device *codec_dev)
{
	...
	it61620->plugged_cb = fn;
	it61620->codec_dev = codec_dev;
	it61620_audio_update_connector_status(it61620);

	return 0;
}

static const struct hdmi_codec_ops it61620_audio_codec_ops = {
	.hw_params = it61620_hdmi_hw_params,
	.audio_shutdown = it61620_hdmi_audio_shutdown,
	.audio_startup = it61620_hdmi_audio_startup,
	.hook_plugged_cb = it61620_hdmi_audio_hook_plugged_cb,
};

static int it61620_hdmi_register_audio_driver(struct device *dev)
{
	struct it61620 *it61620 = dev_get_drvdata(dev);
	struct hdmi_codec_pdata codec_data = {
		.ops = &it61620_audio_codec_ops,
		.max_i2s_channels = 8,
		.i2s = 1,
		.data = it61620,
	};

	it61620->audio_pdev = platform_device_register_data(dev,
							    HDMI_CODEC_DRV_NAME,
							    PLATFORM_DEVID_AUTO,
							    &codec_data,
							    sizeof(codec_data));

	return PTR_ERR_OR_ZERO(it61620->audio_pdev);
}

static void it61620_hdmi_audio_dev_exit(struct it61620 *it61620)
{
	if (it61620->audio_pdev) {
		platform_device_unregister(it61620->audio_pdev);
		it61620->audio_pdev = NULL;
	}
}

static void it61620_plugged_cb_work(struct work_struct *work)
{
	struct it61620 *it61620 = container_of(work, struct it61620,
					       plugged_cb_work.work);
	it61620_audio_update_connector_status(it61620);
}
...
static void it61620_stop_plugged_cb_work(struct it61620 *it61620)
{
	cancel_delayed_work_sync(&it61620->plugged_cb_work);
}
...
static void it61620_hdcp_work(struct work_struct *work)
{
	...
	it61620_start_plugged_cb_work(it61620);
}
				       ...
				       struct it61620 *it61620)
{
	...
	it61620->cec_i2c = devm_i2c_new_dummy_device(dev,
						     client->adapter,
						     CEC_I2C_ADDRESS);
	if (IS_ERR(it61620->cec_i2c))
		return PTR_ERR(it61620->cec_i2c);
	...
	it61620->cec_regmap = devm_regmap_init_i2c(it61620->cec_i2c,
						   &it61620_cec_regmap_config);
	if (IS_ERR(it61620->cec_regmap))
		return PTR_ERR(it61620->cec_regmap);
...
static unsigned int it61620_parse_dt(struct it61620 *it61620)
{
	...
	it61620->en_audio = true;
	it61620->mipirx_config.lane_num = 4;
						       ...
						       struct drm_connector *connector)
{
	...
	if (!it61620->cached_edid) {
		...
		if (!it61620->cached_edid) {
			...
			it61620->is_hdmi = false;
			it61620->en_audio = false;
			return NULL;
		}
		it61620_set_capability_from_edid_parse(it61620, drm_edid_raw(it61620->cached_edid));
	}
...
static int it61620_probe(struct i2c_client *client)
{
	...
	it61620 = devm_kzalloc(dev, sizeof(*it61620), GFP_KERNEL);
	if (!it61620)
		return -ENOMEM;
	...
	INIT_DELAYED_WORK(&it61620->plugged_cb_work, it61620_plugged_cb_work);
	...
	/*register audio*/
	if (it61620->en_audio) {
		ret = it61620_hdmi_register_audio_driver(dev);
		if (ret < 0) {
			dev_err(dev,
	...
	hdmi_node = of_graph_get_remote_node(dev->of_node, 1, -1);
	if (!hdmi_node) {
		dev_err(it61620->dev, "can't find hdmi_node");
		return -ENODEV;
	}

	it61620->next_bridge = of_drm_find_bridge(hdmi_node);
	of_node_put(hdmi_node);

	if (!it61620->next_bridge)
		return -EPROBE_DEFER;

	drm_bridge_add(&it61620->bridge);

	ret = it61620_attach_dsi(it61620, host);
	if (ret < 0) {
		dev_err(it61620->dev, "failed to attach to DSI host: %d", ret);
		return ret;
	}
...
static void it61620_remove(struct i2c_client *client)
{
	...
	it61620_remove_edid(it61620);
	it61620_hdmi_audio_dev_exit(it61620);
	it61620_detach_dsi(it61620);
	drm_bridge_remove(&it61620->bridge);
