...
/*
 * Calculate the total pages for shadow IO page tables for Legacy IOMMU.
 * Similarly to the calculation for shadow EPT, we assume that there is
 * no shared memory between devices using different page tables.
 */
static inline unsigned long pkvm_shadow_iommu_pgtable_pages(int nr_pdev)
{
	unsigned long pgtable_pages = __pkvm_pgtable_total_pages();
	unsigned long res;
